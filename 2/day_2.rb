...
class Day2
  ...
  def self.game_is_possible?(game, set)
    !game.detect do |round|
      round.red > set.red || round.green > set.green || round.blue > set.blue
    end
  end

  def self.sum_games(games, set)
    games.map.with_index(1) do |game, i|
      game_is_possible?(game, set) ? i : 0
    end.sum
  end

  def self.get_result(lines, set)
...
if __FILE__ == $PROGRAM_NAME
  puts Day2.get_result(DATA.readlines, Day2::Set.new(12, 13, 14))
end
