...
class Day1
  def self.get_number(line)
    line.gsub!(/[^0-9]/, "")
    line[0].to_i * 10 + line[-1].to_i
  end

  def self.get_calibration(lines)
    lines.map { |line| get_number(line) }.sum
  end
end

puts Day1.get_calibration(DATA.readlines) if __FILE__ == $PROGRAM_NAME
