...
class Day3
  Coordinate = Struct.new(:line, :char)
  Number = Struct.new(:value, :start)
  Symbol = Number
  Map = Struct.new(:numbers, :symbols)

  def self.count_number(number, symbols)
    top_left = Coordinate.new(number.start.line - 1, number.start.char - 1)
    bottom_right =
      Coordinate.new(
        number.start.line + 1,
        number.start.char + 1 + number.value.to_s.length
      )
    if symbols.detect { |symbol|
         pos = symbol.start
         pos.line <= bottom_right.line && pos.line >= top_left.line &&
           pos.char <= bottom_right.char && pos.char >= top_left.char
       }
      number.value
    else
      0
  ...
  def self.parse_map(lines)
    ...
    lines.each_with_index do |line, line_index|
      line.chomp.chars.each_with_index do |char, char_index|
        if char =~ /\d/
          number_start = Coordinate.new(line_index, char_index) if number.empty?
          number += char
        else
          if !number.empty?
            numbers << Number.new(number.to_i, number_start)
            total += number.to_i
            number = ""
            number_start = nil
          end
          if char != "."
            symbols << Symbol.new(char, Coordinate.new(line_index, char_index))
          end
        end
      end
    end
    puts "TOTAL: #{total}"
    Map.new(numbers, symbols)
  end

  def self.get_result(lines)
    map = parse_map(lines)
    map.numbers.map { |number| count_number(number, map.symbols) }.reduce(:+)
  end
end

puts Day3.get_result(DATA.readlines) if __FILE__ == $PROGRAM_NAME
